...
La première réponse pour notre problème est de positionner verrou lors du `SELECT` et faire un `SELECT ... FOR UPDATE`
...
![plocking](plocking.png)

Par conséquence, 
1. Lorsque Bob voudra accéder à la ligne il doit attendre que le verrou soit levé
2. Donc lorsqu'il fera explicitement son `SELECT` il aura `stock=0`
3. Donc le test en java `if(qty<0)` échoue
