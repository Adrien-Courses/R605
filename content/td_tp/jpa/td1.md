...
- Compléter le code de la méthode `getSameEntityTwice()` pour récupérer deux fois la même entité, que remarquez-vous ?
  - est-ce qu'un nouvel appel à la base de données est exécuté ?
  - quelle est la valeur de la référence après le second appel ? 

<br>

- Bloquer le Thread pendant plusieurs secondes le temps de mettre à jour directement dans la base de données une nouvelle valeur pour la description du cours
  ```java
...
- Récupérer une première fois l'entité en base de données puis détachez-la.
- Récupérer une seconde fois cette même entité, que remarquez-vous ?


### 2.3 Re-attacher une entité détachée
Si on modifie une entité `DETACHED` et qu’on souhaite persister ses nouvelles valeurs nous devons la rattacher pour que l’EntityManager persiste son état. Mais que deviennent les entités ?
